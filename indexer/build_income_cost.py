...
NEW_CFG = (
    '/* ═══ P&L 매트릭스 초안 (원칙 기반 접두어 매칭) ═══\n'
    '   회사별 세부 코드는 UI의 [룰 설정 JSON] 편집으로 조정 가능.\n'
    '   근거: 회계분리기준 고시 §16(수익 배부)·§17(비용 배부) + 해설서 2026.4 */\n'
    'var DEFAULT_CFG = {\n'
    '  /* 계정 접두어 → 허용 형태 접두어. 계정 체계(4xx수익, 5xx원가, 6xx판관비 등) */\n'
    '  ACCT_FORM: {\n'
    '    /* ─── 수익 계정 (4xxxxx) ─── */\n'
...
new_html = new_html.replace(
    ...
    '{id:"R2", sev:"HIGH", name:"계정과목 ↔ 형태코드 불일치 (재무회계 준용)", basis:"재무 P&L 계정과 통신 형태코드의 연쇄 불일치. 원칙: 수익 계정(4xx)→수익형태(FC7·), 원가 계정(5xx)→설비운영비용(FC5·), 판관비(6xx)→판관비형태(FC6·). 형태·기능만 정정하고 계정은 유지."}', 1)
new_html = new_html.replace(
...
new_html = new_html.replace(
    ...
    'ACCT_FORM: 계정과목(앞자리 매칭) → 허용 형태코드 접두어 (수익 4xx / 원가 5xx / 판관비 6xx).\n'
